...
//    log(LOG_DEBUG, szMessage);
    ...
    {
        log(LOG_DEBUG, "CheckOverloadCount() return false, we'll return false.");
        return(false);
    }

    // An image for highlighting detected objects.
    Image *pMaskImage = new Image(zmImage->Width(), zmImage->Height(), 1);
    ...
    if (foundObjects.size() > 0)
        log(LOG_DEBUG, "OBJECTS WERE DETECTED");
	...
	Image *hlZmImage = pMaskImage->HighlightEdges(alarm_colour, &zone_polygon.Extent());
